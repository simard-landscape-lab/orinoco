...
def get_width_features_from_segments(label_array: np.ndarray, profile: dict) -> np.ndarray:
    """
    Takes a label array and obtains width using the distance transform in a neighborhood of each segment.

    Specifically, the width within a segment is determined as `(2*d - 1) * res`, where `d` is the maximum of the distance
    transform in that segment where we compute the distance transform in a small bounding box of the segment and the
    `res` is the resolution (in meters; we assume all rasters are in meters) determined using the rasterio profile.

    Assume label 0 is land and channel_mask is (label_array != 0).

    We restrict calculations to 1 pixel buffer of bounding box around segment. `get_width_features_from_segments_naive`
    uses the distance transform on the full channel mask.
    ...
    -------
    np.ndarray:
        Obtain features of shape (p x 1) where `p:= unique labels in label array` and index i of feature vector corresponds
        to width of label i.

    Notes
    -----
    + width at label 0 will be set to np.nan
    + If distance transform at particular segment is 0 then we assign width at node the width determined using the
    full distance transform determined using the channel mask, i.e.  (label_array != 0) not just the distance transfom
    in a buffered area around the segment.
    """
...
def get_width_features_from_segments_naive(label_array: np.ndarray,  profile: dict) -> np.ndarray:
    """
    Takes a label array and obtains width using the distance transform using the entire channel mask (label_array != 0).

    Specifically, the width within a segment is determined as `(2*d - 1) * res`, where `d` is the distance transform determined
    using the the entire channel mask.

    Assume label 0 is land and channel_mask is (label_array != 0).

    Contrast this with get_width_features_from_segments which computes distance transform only within a bounding box of the segment.
...
def add_width_features_to_graph(G: nx.classes.graph.Graph,
                                ...
                                width_label: str = 'width_from_segment') -> nx.Graph:
    """
    Take width features of length p in which index has i has width at that label and add that as node attribute.
    Width features should be flattened (or width_features.ravel())

    We take the node_data = {node: data_dict for node in G.nodes()} and update each data_dict with `width_label`: width.
...
def angle_between(v1: np.ndarray, v2: np.ndarray) -> float:
    """
    Find the angle between two vectors using the arccosine.

    Specifically, arcos( v1 * v2 ) / (||v1||_2 ||v2||_2), where * indicates the vector dot product.
...
def realign_vector(edge: np.ndarray, reference_edge: np.ndarray) -> np.ndarray:
    """
    Using the reference_edge, ensure that the other edge is within pi/2.

    If not, take its negative aka reverse the vector aka reflect through origin.
